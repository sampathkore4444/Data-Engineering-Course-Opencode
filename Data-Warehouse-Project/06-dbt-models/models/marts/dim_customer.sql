...
{{
    config(
        ...
        post_hook=[
            "CREATE INDEX IF NOT EXISTS idx_dim_customer_id ON {{ this }} (customer_id)",
            "CREATE INDEX IF NOT EXISTS idx_dim_customer_type ON {{ this }} (customer_type)"
        ]
    )
}}
...
-- Add surrogate key and SCD columns
final as (
    select
        -- Surrogate key
        ...
        -- Customer segmentation based on balance
        case
